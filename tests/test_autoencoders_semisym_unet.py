...
from atompaint.autoencoders.semisym_unet import SemiSymUNet_EncDecChannels, SemiSymUNet_DownUpChannels
...
def test_semisym_unet_enc_dec_channels(kwargs):
    gspace = rot3dOnR3()
    ...
    unet = SemiSymUNet_EncDecChannels(
            img_channels=3,
...
def test_semisym_unet_down_up_channels(kwargs):
    gspace = rot3dOnR3()
    so3 = gspace.fibergroup
    grid = so3.grid(type='thomson_cube', N=4)
    ...
    def downsample_factory(in_type, out_type):
        yield R3Conv(
                in_type,
                out_type,
                kernel_size=3,
    ...
    unet = SemiSymUNet_DownUpChannels(
            img_channels=3,
            encoder_types=make_fourier_field_types(gspace, [1, 2, 3], 2),
    ...
    x = torch.randn(2, 3, 11, 11, 11)
    y = torch.randn(2)
    xy = torch.randn(2, 3, 11, 11, 11)

    if kwargs.get('allow_self_cond'):
        x_self_cond = torch.randn(2, 3, 11, 11, 11)
    else:
        x_self_cond = None
