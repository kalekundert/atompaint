...
from atompaint.autoencoders.sym_unet import SymUNet, SymUNet_DownUpChannels, SymUNetBlock
...
def test_sym_unet_block_channels(kwargs):
    gspace = rot3dOnR3()
...
def test_sym_unet_down_up_channels(kwargs):
    gspace = rot3dOnR3()
    so3 = gspace.fibergroup
    grid = so3.grid(type='thomson_cube', N=4)
    ...
    def upsample_factory(in_type, out_type):
        yield R3ConvTransposed(
                in_type,
                out_type,
                kernel_size=3,
                padding=1,
